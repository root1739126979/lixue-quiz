...
个人口径数据不再放在整体看板中。管理员进入“员工管理”后，点击某一员工的“查看”，可查看该员工的累计答题、整体正确率、总积分和未掌握错题。
...
git remote add origin https://github.com/root1739126979/lixue-quiz.git
...
- 等待并检查后端 `http://127.0.0.1:8000/api/health`。
- 等待并检查前端 `http://127.0.0.1:8080`。
- 如果构建失败，或前后端在等待后仍不可用，回退到更新前版本。

健康检查默认最多等待约 60 秒。如果服务器较慢，可以这样延长等待时间：
